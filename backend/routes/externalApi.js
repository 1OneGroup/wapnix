...
router.post('/send', async (req, res) => {
  try {
    const { phone, message, template_id, variables } = req.body;
    ...
    if (sent >= req.user.daily_limit) return res.status(429).json({ error: 'Daily message limit reached' });
    ...
    const { media } = req.body; // optional array of { data: base64, mimetype, caption }
    ...
    const sendAll = async () => {
      // Send images if provided (base64 data)
...
router.post('/send-bulk', async (req, res) => {
  try {
    ...
    if (remaining <= 0) return res.status(429).json({ error: 'Daily message limit reached' });
    ...
    (async () => {
      for (const r of queued) {
        try {
          ...
          // Send images if provided (base64), otherwise text only
          if (media && Array.isArray(media) && media.length > 0) {
            for (let i = 0; i < media.length; i++) {
              const item = media[i];
              if (!item.data) continue;
...
router.get('/templates', (req, res) => {
  const templates = db.prepare('SELECT id, name, body, variables, category FROM templates WHERE user_id = ?').all(req.user.id);
  res.json({ templates });
});
